...
#     data = data.drop_duplicates(subset=["start_utc","end_utc", "solar_forecast_kw"])
#     deduplicated_row_count = len(data)

#     logger.info(
#         f"Removed {initial_row_count - deduplicated_row_count} duplicate rows based on 'start_utc','end_utc' and 'solar_forecast_kw'."
#     )
...
def format_to_forecast_sql(data: pd.DataFrame, model_tag: str, model_version: str, session) -> list:
    ...
    # Log initial data snapshot
    logger.debug(f"Initial data preview:\n{data.head()}")
    logger.info(f"Initial data row count: {len(data)}")
    ...
    logger.debug(f"Deduplicated data preview:\n{data.head(10)}")
    logger.info(f"Deduplicated data row count: {deduplicated_row_count}")
